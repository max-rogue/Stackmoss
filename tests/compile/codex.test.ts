...
describe('Codex Compile Target', () => {
    it('generates AGENTS.md and Codex project skills', () => {
        ...
        const bootstrap = files.find((file) => file.path === '.agents/skills/stackmoss-bootstrap/SKILL.md');
        const methodology = files.find((file) => file.path === '.agents/skills/methodology/SKILL.md');
        ...
        expect(methodology!.content).toContain('name: methodology');
        expect(methodology!.content).toContain('This is the shared methodology layer for Codex');
        expect(bootstrap!.content).toContain('Treat `AGENTS.md` as the repo-level Codex instruction file');
    });
});
